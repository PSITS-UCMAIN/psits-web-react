...
description: Disciplined end-to-end code quality orchestration. Runs linting, code review, full tests, delegates fixes to small-fix-agent and engineered-implementor-agent. Routes complex work by scope. RECOMMENDED to Run under coordinator agent mode for full delegation capability.
user-invocable: true
---
...
- **`engineered-implementor-agent`** — Complex, multi-file implementations requiring planning
...
2. `/code-review` — Project-aware code review against `.agents/rules/coding-rules.md`
3. `/full-tests` — Complete Vitest suite execution across `server-side/` and `client-side-ts/`
4. Agent delegation — `small-fix-agent` (simple) or `engineered-implementor-agent` (complex)
...
| Multi-file, or requires planning | `engineered-implementor-agent` | **Coordinator mode** |
| Architectural, high-risk, or systemic | ⚠️ Flag for human review | N/A |

**If not in coordinator mode and a task requires `engineered-implementor-agent`:**
> ⚠️ **Human action required**: This task requires coordinator agent mode. Please switch to coordinator mode and re-invoke this skill, or manually assign this task to `engineered-implementor-agent`.
...
- Agent definitions in `.agents/agents/` and rules in `.agents/rules/`
...
3. If in **default mode**: 
   ...
   - Continue execution. Flag any task requiring `engineered-implementor-agent` as a 
...
5. Delegate safe fixes:
   ...
   - **Multi-file linting refactors** → `engineered-implementor-agent` (coordinator mode only).
...
### Phase 2 — Code Review (`/code-review`)
...
5. Record low-risk items for Phase 3 implementation.

---

### Phase 3 — Implement Low-Risk Fixes
...
**Multi-file or complex fixes → `engineered-implementor-agent`:**
...
3. Triage every failure:
   ...
   **A — Test infrastructure errors** (missing setup, missing env vars, broken harness):
   - `engineered-implementor-agent` (coordinator mode) or ⚠️ flag (default mode)

   **B — Test logic errors** (stale assertions, test broken after code change):
   - `small-fix-agent` if isolated; `engineered-implementor-agent` if multi-file (coordinator mode) or ⚠️ flag (default mode)
   ...
   **C — Source code issues surfaced by tests** (bugs, regressions):
   ...
   - Multi-file or systemic → **STOP**. Flag with: test name, affected module, reason, 
     recommended next step. Do NOT attempt the fix. Proceed to Phase 5.

4. Re-run `/full-tests` after all fixes to confirm clean state.
5. If suite cannot reach passing state, document remaining failures and proceed to Phase 5.
...
**Body sections:**
- **Summary** — Overall outcome: lint status, code review verdict, test pass rate, escalations.
...
- **Code Review Results** — Verdict, low-risk items implemented, high-risk items flagged.
...
1. Invoke `/git-committer-atomic` skill against all changes from Phases 1–5.
...
| 2 | Code Review | Accept / Request Changes / Block | verdict |
| 3 | Implement Low-Risk Fixes | Complete / Partial / Blocked | files changed |
| 4 | Full Tests | Pass / Partial / Fail | failures fixed, escalated |
| 5 | Report Generated | ✅ | path to report file |
...
- **Agents**: 
  ...
  - `engineered-implementor-agent` — complex multi-file work (coordinator mode only)
- **Reference docs**: `.agents/rules/` (conventions), `.agents/agents/` (agent definitions)
...
- Lint and test failures triaged and routed to `small-fix-agent` or `engineered-implementor-agent`
